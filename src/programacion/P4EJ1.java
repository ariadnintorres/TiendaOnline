...
public class P4EJ1 {
	...
	public static void main(String[] args) {
		...
		do {
			...
			case 7: // **NUEVO: ELIMINAR ARTÍCULO DEL CARRITO**
				// Llamada a la nueva función (se pasa el array 'precios')
				numArticulosCarrito = eliminarArticuloDelCarrito(sc, nombres, precios, carritoIDs, carritoCantidades, numArticulosCarrito);
				break;

			case 8: // VER CARRITO ACTUAL (Antigua Opción 7)
				verCarrito(nombres, precios, carritoIDs, carritoCantidades, numArticulosCarrito);
				break;

			case 9: // FINALIZAR COMPRA Y PAGAR (Antigua Opción 8)
	...
	// SINA B4: Eliminar Artículo del Carrito (Opción 7 - NUEVA)
	/**
	 * Permite reducir la cantidad de un producto o eliminar completamente una línea del carrito.
	 * Retorna el nuevo número de artículos diferentes en el carrito (numArticulosCarrito).
	 */
	public static int eliminarArticuloDelCarrito(Scanner sc, String[] nombres, double[] precios, int[] carritoIDs, int[] carritoCantidades, int numArticulosCarrito) {

		System.out.println(RED + "\n--- Eliminar Artículo del Carrito ---" + RESET);

		// 1. Chequeo inicial (Corregido a == 0)
		if (numArticulosCarrito == 0) {
			System.out.println(YELLOW + "El carrito está vacío. No hay artículos para eliminar." + RESET);
			return 0; 
		}

		// 2. Mostrar carrito actual para seleccionar la LÍNEA (Usamos verCarrito para el desglose)
		verCarrito(nombres, precios, carritoIDs, carritoCantidades, numArticulosCarrito);

		// --- 3. PEDIR LÍNEA A ELIMINAR ---
		int lineaEliminar = -1;

		// Validar que la línea esté entre 1 y el número de artículos
		while (lineaEliminar < 1 || lineaEliminar > numArticulosCarrito) {
			// Es importante usar el mismo Scanner sc que se usa para pedirEnteroSeguro
			lineaEliminar = pedirEnteroSeguro(sc, YELLOW + "Introduce la LÍNEA (número) del artículo a ELIMINAR [1 - " + numArticulosCarrito + "]: " + RESET);

			if (lineaEliminar < 1 || lineaEliminar > numArticulosCarrito) {
				System.out.println(RED + "Línea inválida. Introduce un número entre 1 y " + numArticulosCarrito + "." + RESET);
			}
		}

		int indiceCarrito = lineaEliminar - 1; // Convertir a índice 0-based
		int idProducto = carritoIDs[indiceCarrito];
		int cantidadActual = carritoCantidades[indiceCarrito];
		String nombreProducto = nombres[idProducto];

		// Consumir el salto de línea pendiente antes de la siguiente lectura
		sc.nextLine(); 

		// --- 4. PEDIR CANTIDAD A ELIMINAR ---
		int cantidadEliminar = -1;

		// Validar que la cantidad esté entre 1 y la cantidad actual
		while (cantidadEliminar < 1 || cantidadEliminar > cantidadActual) {
			cantidadEliminar = pedirEnteroSeguro(sc, YELLOW + "Cantidad de '" + nombreProducto + "' a eliminar [1 - " + cantidadActual + "]: " + RESET);

			if (cantidadEliminar < 1) {
				System.out.println(RED + "Debes eliminar al menos 1 unidad." + RESET);
			} else if (cantidadEliminar > cantidadActual) {
				System.out.println(RED + "No puedes eliminar más de las " + cantidadActual + " unidades que tienes en esa línea." + RESET);
			}
		}

		sc.nextLine(); // Consumir salto de línea después del último pedirEnteroSeguro

		// --- 5. REALIZAR LA MODIFICACIÓN O ELIMINACIÓN ---

		if (cantidadEliminar == cantidadActual) {
			// Eliminar la línea completa (Desplazamiento a la izquierda)
			for (int i = indiceCarrito; i < numArticulosCarrito - 1; i++) {
				carritoIDs[i] = carritoIDs[i + 1];
				carritoCantidades[i] = carritoCantidades[i + 1];
			}

			// Limpieza del último elemento (buena práctica)
			carritoIDs[numArticulosCarrito - 1] = 0;
			carritoCantidades[numArticulosCarrito - 1] = 0;

			numArticulosCarrito--;
			System.out.println(GREEN + " Artículo '" + nombreProducto + "' (Línea " + lineaEliminar + ") ELIMINADO COMPLETAMENTE del carrito." + RESET);

		} else {
			// Reducir la cantidad
			carritoCantidades[indiceCarrito] -= cantidadEliminar;
			System.out.println(GREEN + "Reducidas " + cantidadEliminar + " unidades de '" + nombreProducto + "'. Cantidad restante: " + carritoCantidades[indiceCarrito] + "." + RESET);
		}

		return numArticulosCarrito;
	}
}
